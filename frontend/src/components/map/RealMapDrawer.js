...
import { MapPin, Square, ZoomIn, ZoomOut, Move, Trash2, MousePointer } from 'lucide-react';

const RealMapDrawer = ({ onAOIChange, initialBounds = null, className = "" }) => {
  ...
  useEffect(() => {
    ...
    // Create the map HTML structure with OpenStreetMap tiles
    ...
    const mapElement = mapRef.current.querySelector('#osm-map');
    ...
    <div className={className}>
      <div className="bg-slate-700 rounded-lg p-4 h-[400px] flex flex-col">
          ...
          {aoiBox && (
            <div className="absolute top-2 right-2 bg-black bg-opacity-90 rounded-lg px-3 py-2 border border-blue-500">
              <div className="text-xs text-white space-y-1">
                ...
                <div className="text-gray-300">Street view • Z{zoom}</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-2 text-xs text-gray-400 text-center">
          🗺️ OpenStreetMap tiles • Click and drag to create AOI rectangle
        </div>
      </div>
    </div>
  );
